...
2. **校准** —— 实测每种透明耗材真实的背光颜色，构建可打印颜色的查找表（LUT）与色域
   （见 [`filament/`](filament/读我.md)）；还可 **导出可打印颜色**（`.gpl`/`.aco`），
   用真正能打印出来的调色板作画。
...
python3 filament_gui.py    # 校准耗材 → LUT/色域          → http://127.0.0.1:8000
```
...
由 `python3 scripts/png_to_stained_glass_svg.py <输入>.png --preview` 生成。
点击图片可查看原始分辨率。
...
```bash
python3 scripts/png_to_stained_glass_svg.py input.png [选项]
```
...
| 选项 | 默认 | 说明 |
|------|------|------|
| `--link-lines` | 关 | 描边模式。**简单（默认，关）**＝逐弧笔画，各按自身实测宽度分级，最为一致。**高级（`--link-lines`）**＝将接缝弧段在交点处连成连续笔画；用于繁复线稿、希望把被多次交叉打断的线保持为一条。下面两项用于调节高级模式。 |
| `--link-angle DEG` | `35` | （需 `--link-lines`）在交点处，仅当两弧段切线方向偏离直线小于此角度时才连成一条连续线（角度越大越算作拐角，链条在此断开）。 |
| `--link-width-ratio R` | `1.7` | （需 `--link-lines`）将两弧段视为同一条线的最大宽度比。 |
...
运行 `python3 scripts/png_to_stained_glass_svg.py --help` 查看完整选项列表。
